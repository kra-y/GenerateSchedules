...
RoomBlockPermutations<-function(Block){
  ...
  perms<-permutations(n=numberofclasses,r=numberofclasses,characters[[1]],set = F)
  return(perms) 
...
DurationCode<-function(Ablocks,Bblocks,Cblocks){
  DURATION_CODE<- paste0(c(rep("A",Ablocks),rep("B",Bblocks),rep("C",Cblocks)),collapse = "")
...
for(i in 1:length(BuildingA$classroom)){
  BuildingA$ACLASSES[i]<-sample(1:13,1,replace = T)
...
# but also have 2 or 3 other classes along with math 980 throughout the day
df1<-BuildingA%>%
  # group_by(classroom)%>%
